...
function ferraris () {
  const filterFerraris = cars.filter(car => car['brand'] === "Ferrari" ?? car)
  return filterFerraris
}

console.log("Ferrari", ferraris());

function nineties() {
  const ninetiesCars = cars.filter(car => car.model >= 1980 && car.model <= 1990 ? car : null)
  return ninetiesCars
}

console.log("1980-1990", nineties());
...
function bmwSum() {
  const listOfBMW = cars.map(car => car.brand === 'BMW' ? car.price : 0)
  const totalPrice = listOfBMW.reduce((price, currentValue) => price + currentValue, 0)

  return totalPrice
}

console.log('Price', bmwSum());